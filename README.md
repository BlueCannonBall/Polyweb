...
server.route("/hello_world",
    pw::Route {
        [](const pw::Connection& conn, const pw::Request& req) {
...
server.route("/multiply",
    pw::Route {
        [](const pw::Connection& conn, const pw::Request& req) {
            ...
            return pw::Response(200, std::to_string(x * y), {{"Content-Type", "text/plain"}});
        },
    });
    ...
    pw::Route {
        [](pw::Connection& conn, pw::RequestReceiver& req) {
            ...
            if (pn::Status result = conn.recv(req, PW_HTTP_MESSAGE_PART_BODY); !result) {
                return pw::Response(500, result.error().message());
            }

            return pw::Response(200, body);
        },
        false,
